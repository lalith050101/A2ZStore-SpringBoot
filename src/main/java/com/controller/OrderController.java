	...
	public ModelAndView placeOrder(ModelAndView mandv, HttpServletRequest request, Authentication authentication) {
		...
		if (paymentType.equals("OnlinePayment")) {
			...
			try {
				...
				Order order = new Order(orderModel.getTotalPrice(), "USD", "PayPal", "sale",
						orderModel.getId().toString());

				Payment payment = payPalService.createPayment(order.getPrice(), order.getCurrency(), order.getMethod(),
						order.getIntent(), order.getDescription(), APP_URL + CANCEL_URL, APP_URL + SUCCESS_URL);
